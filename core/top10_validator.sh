...
# 6 - ShellCheck disables found (warn only if explicitly disabled)
if grep -Eq '^[[:space:]]*# shellcheck disable=' "$SCRIPT"; then
...
# 4 - File listing: Uses find/globs, avoids ls (code-only check)
if grep -E '^[[:space:]]*[^#]*\bls\b(?![^"]*")' "$SCRIPT" | grep -vE '^[[:space:]]*#' >/dev/null 2>&1; then
  STATUS[4]="${color_red}${CROSS} - [B01] -${color_reset}"; LEVEL[4]="bad"
elif grep -Eq '\bfind\b|\*[^ ]*' "$SCRIPT"; then
